...
def repo():
    try:
        ...
        if i == 956014:
            ...
            f = "data11.csv"
        if (int(years.get()) <= 2016) or (int(years.get()) > 2021):
            messagebox.showerror(title="Error", message="Please Enter correct values years range should be(2017-2021)",
                                 parent=window)
        else:
            myfilenew.predict_data(f, mon.get(), int(years.get()))
    except:
        messagebox.showerror(title="Error",
                             message="Please Enter correct values year's range should be(2017-2021)\nPlease Enter correct Month First Letter Should be Capital",
                             parent=window)
...
ptitle = Label(window, text='SALES FORECASTING FOR GROCERY STORE PERISHABLE FOOD PRODUCTS USING MACHINE LEARNING',
               fg="red", bg="white")
