...
def predict_data(filen, mnth, yrs):
    ...
    if len(ys) == len(years):
        ...
        print(reg_line)
        predict = (m * yrs) + b
    print("combine list of monthly sale of " + mnth + " in " + str(
        years) + " using Rolling Forcast with Exponential Smoothing Method")
    print(ys)
    print("Predicted sale of " + mnth + " in " + str(
        yrs) + " using Linear Regrission based on combine list of monthly sale using Rolling Forcast with Exponential Smoothing Method =" + str(
        predict))
